...
function createModelModule() {
    ...
    _.extend(ImageModel.prototype, {
        ...
        setRating: function(rating) {
            if (!(_.isNumber(rating) && rating >= 0 && rating <= 5)) {
                throw new Error("Invalid arguments to ImageModel.setRating: " + JSON.stringify(arguments));
            }
            this.rating = rating;
            var self = this;
            var eventTime = new Date();
    ...
    var ImageCollectionModel = function() {
        ...
        this.listeners = [];
        var self = this;
        this.imageListenerFn = function (imageModel, eventTime) {
            _.each(
                self.listeners,
                function (listener_fn) {
    ...
    _.extend(ImageCollectionModel.prototype, {
        ...
        addImageModel: function(imageModel) {
            ...
            imageModel.addListener(this.imageListenerFn);
        ...
        removeImageModel: function(imageModel) {
            imageModel.removeListener(this.imageListenerFn);
     ...
     */
    function loadImageCollectionModel(imageCollectionModel) {
